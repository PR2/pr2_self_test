...
class JointParamChanger:
    ...
    def __init__(self, whicharm): #whicharm is 'r' or 'l'
        ...
        self.default_p = [400., 1200., 1000., 700., 300., 300., 300.]
        self.default_d = [18., 10., 6., 4., 6., 4., 4.]
        self.default_i = [10., 70., 60., 45., 30., 30., 30.]
        ...
        joint_names = ['_shoulder_pan_joint', '_shoulder_lift_joint',
                         ...
                         '_wrist_roll_joint']
        self.controller_name = "both_arms_controller"
    ...
    def set_gains_fraction(self, side, fraction):
        for i in range(7):
            rospy.set_param(self.controller_name+'/gains/%s%s/p'%(side, self.joint_names[i]), self.default_p[i]*fraction)
            rospy.set_param(self.controller_name+'/gains/%s%s/d'%(side, self.joint_names[i]), self.default_d[i]*fraction)
            rospy.set_param(self.controller_name+'/gains/%s%s/i'%(side, self.joint_names[i]), self.default_i[i]*fraction)
            rospy.set_param(self.controller_name+'/gains/%s%s/i_clamp'%(side, self.joint_names[i]), self.default_i_clamp[i]*fraction)
            

    #unload and reload the joint controllers with the params on the server
    def reload_joint_controllers(self, fraction = 1.):
        self.set_gains_fraction('r', fraction)
        self.set_gains_fraction('l', fraction)
    ...
    def load_joint_controllers(self):
        success = self.load_controller_service(self.controller_name)
        if not success:
    ...
    def unload_joint_controllers(self):
        ...
        success = self.unload_controller_service(self.controller_name)
        if not success:
    ...
    def stop_joint_controllers(self):
        success = self.switch_controller_service([], [self.controller_name,], 2)
        if success:
